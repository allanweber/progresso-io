...
export const ADMIN_INVITE_EMAIL_SUBJECT =
  "Convite de administrador — Progresso IO";

type AdminInviteEmailProps = {
  /** Invitee's first name, for the greeting. */
  firstName: string;
  /** Absolute URL to the admin accept page (carries the raw token). */
  acceptUrl: string;
  /** Days until the invite expires, for the copy. */
  expiresInDays: number;
};

/** Branded admin-invite e-mail: a single CTA that opens the set-password page. */
export function AdminInviteEmail({
  ...
  return (
    <Html lang="pt-BR">
      <Head />
      <Preview>Você foi convidado como administrador do Progresso IO</Preview>
      <Body style={main}>
        <Container style={container}>
          <Text style={brand}>
            Progresso <span style={brandAccent}>IO</span>
          </Text>
          <Heading style={heading}>Convite de administrador</Heading>
          <Text style={paragraph}>
            Olá, {firstName}! Você foi convidado para administrar a plataforma
            Progresso IO. Defina sua senha para ativar seu acesso de
            administrador.
          </Text>
