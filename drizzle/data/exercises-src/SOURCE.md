...
1. `npm run db:transform-exercises` — joins the two files on `id`, keeping PT-BR
...
2. `npm run db:fetch-exercise-images` — downloads every image referenced by the
...
3. `npm run db:upload-exercise-images` — uploads that local image folder to the
