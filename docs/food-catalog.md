...
4. **Substitutions** (`drizzle/data/taco-substitutions.json`, seeded last) — ~274
   shared **base** substitution rules (`clinic_id NULL`) generated by
   `scripts/build-substitutions.mjs` from within-role groups (grains, lean
   proteins, legumes, fats, dairy, nut butters, seeds, fruits, sweeteners,
   flours…). Each is directed — `grams` of the substitute ≡ 100 g of the main
   food — and sized **isocalorically** (`grams = 100·kcal_main/kcal_sub`). Foods
   are referenced by their exact `description`, resolved to ids at seed time; a
   rule whose food isn't in the catalog is skipped. Idempotent: skips once any
   base substitution exists.
...
- `scripts/build-substitutions.mjs` → `drizzle/data/taco-substitutions.json` —
  the base substitution rules (foods referenced by description).
- `scripts/migrate.mjs` — migrations + idempotent catalog, supplement &
  substitution seed.
